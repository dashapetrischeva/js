...
if (confirm('Start testing?')) {
    ...
    const totalPrice = sites.reduce((prevSum, site) => prevSum + site.sitePrice, 0);
    document.write(` <div>1) Total price of all websites: <strong>${totalPrice}</strong></div>`);

    const sitesNumber20002009 = sites.reduce((prevCount, site) => ((site.releaseYear >= 2000 && site.releaseYear <= 2009) ? prevCount + 1 : prevCount), 0);
    document.write(` <div>2) Number of websites developed between 2000 and 2009: <strong>${sitesNumber20002009}</strong></div>`);

    const sitesNumberInvestmentAmountMore100000 = sites.reduce((prevCount, site) => {
    ...
    document.write(` <div>3) Number of websites where the total sponsorship investment was more than 100000: <strong>${sitesNumberInvestmentAmountMore100000}</strong></div>`);
    ...
    document.write(` <div>5) Year when profit was the highest: <strong>${maxYear}</strong></div>`);
    ...
    let arrSitesLess10000 = sites
        ...
        .map(site => (Object.assign(Object.assign({}, site), { sponsors: [...site.sponsors] })));
